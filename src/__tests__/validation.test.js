...
describe('Quiz', () => {
  ...
  it('should be able to answer a question', async () => {
    ...
    const answer = await page.$eval('[data-test="pergunta"]', question => question.dataset.resposta);

    assert(['correta', 'errada'].includes(answer), 'Question should have a data attribute "resposta" with the answer.');
  });
  ...
  it('should show correct answers count and reset button', async () => {
    ...
    const choices = await Promise.all(
      questions.map(async (question, index) => {
        ...
        assert(options.length > 1, `Question number "${index + 1}" should have more than one option.`);
    ...
    assert((await page.$(`[data-resultado="${correct}"]`)) !== null, 'Can not assert correct answers count.');
    ...
    assert((await page.$(`[data-resultado="${correct}"]`)) === null, 'Correct answers score should be reseted after clicking reset button.');

    const answers = await page.$$eval('[data-test="pergunta"]', questions => questions.map(question => question.dataset.resposta));
    
    assert(answers.filter(answer => ['correta', 'errada'].includes(answer)).length === 0, 'Question data attribute "resposta" should be reseted after clicking reset button.');
  });
});
